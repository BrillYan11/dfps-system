...
if ($selected_conv_id) {
    ...
    $read_stmt->bind_param("ii", $selected_conv_id, $buyer_id);
    $read_stmt->execute();
    $read_stmt->close();
}
...
if ($receiver_id && !$selected_conv_id) {
    ...
    ");
    $conv_lookup_stmt->bind_param("ii", $buyer_id, $receiver_id);
    ...
    if ($conv_row = $conv_lookup_result->fetch_assoc()) {
        ...
        $conn->query("UPDATE conversation_participants SET is_archived = 0 WHERE conversation_id = $selected_conv_id AND user_id = $buyer_id");
    } else {
        ...
        try {
            ...
            $part_stmt->bind_param("iiii", $selected_conv_id, $buyer_id, $selected_conv_id, $receiver_id);
            $part_stmt->execute();
            $part_stmt->close();
            $conn->commit();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty(trim($_POST['message_body'])) && $selected_conv_id) {
    ...
    try {
        ...
        $verify_stmt->bind_param("ii", $selected_conv_id, $buyer_id);
        $verify_stmt->execute();
        ...
        if ($result->num_rows > 0) {
            ...
            $msg_stmt->bind_param("iis", $selected_conv_id, $buyer_id, $message_body);
            $msg_stmt->execute();
            $msg_stmt->close();

            // Notify Receiver
            $me_stmt = $conn->prepare("SELECT first_name, last_name FROM users WHERE id = ?");
            $me_stmt->bind_param("i", $buyer_id);
            $me_stmt->execute();
...
$conv_stmt->bind_param("iiii", $buyer_id, $buyer_id, $buyer_id, $is_archived_filter);
$conv_stmt->execute();
...
if ($selected_conv_id) {
    $p_stmt = $conn->prepare("
        ...
        WHERE other_user.id != ?
    ");
    $p_stmt->bind_param("iii", $selected_conv_id, $buyer_id, $buyer_id);
    $p_stmt->execute();
            ...
            <div class="chat-header justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <small class="text-success" style="font-weight: 600; font-size: 0.75rem;">Farmer</small>
                    </div>
                </div>
                 ...
                 <?php if (empty($messages)): ?>
                    <div class="text-center text-muted my-auto">
                        <?php if ($post_id):
                            ...
                            $post_stmt->close();
                        ?>
                            <div class="alert alert-light border shadow-sm mx-auto" style="max-width: 400px;">
                                Start the conversation about <strong>"<?php echo htmlspecialchars($post_title); ?>"</strong>.
                            </div>
                        <?php else: ?>
                            <p>No messages here yet. Say hi!</p>
                        <?php endif; ?>
                    </div>
                 <?php else: ?>
                    <?php foreach ($messages as $message):
                        $isSent = ($message['sender_id'] == $buyer_id);
...
<script>
    var currentUserId = <?php echo $buyer_id; ?>;
